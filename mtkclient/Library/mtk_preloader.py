...
class Preloader(metaclass=LogBase):
    ...
    def init(self, display=True, directory: str = None):
        ...
        if meid is not None:
            ...
            self.config.hwparam = HwParam(self.mtk.config, "", self.mtk.config.hwparam_path)
        if meid is not None:
    ...
    def setreg_disablewatchdogtimer(self, hwcode, hwver):
        ...
        if hwcode in [0x2625, 0x2523, 0x7682, 0x7686, 0x5932]:
            self.write16(0xA2050000, 0x2200)
            res = True
        elif hwcode == 0x6261:
            # Disable watchdog timer
            # MT2503
            if hwver in [0xca02, 0xcb01]:
                """
                self.write16(0x80000000, 0x1)
                self.write16(0x80000008, 0x1)
                self.write16(0x8000000C, 0x1)
                self.write16(0x80000000, 0x1)
                self.write16(0x80000004, 0x1)
                self.write16(0xA01C0108, 0x2)
                ##
                self.read32(0xA01C0108)
                self.write16(0xA0700A28, self.read16(0xA0700A28) | 0x4000)
                self.write16(0xA0700A00, self.read16(0xA0700A00) | 0x1)
                self.write16(0xA0030000, 0x2200)
                self.read16(0xA0710000)
                self.read16(0xA0710050)
                self.read16(0xA0710054)
                ...
                self.write16(0xA0710074, 0x0001)
                self.read16(0xA0710000)
                self.write16(0xA0710071, 0x430E)
                self.write16(0xA0710074, 0x0001)
                self.read16(0xA0710000)
                # D8
                self.write32(0xA0510000, self.read32(0xA0510000) | 2)
                self.write32(0xA0510000, self.read32(0xA0510000) | 2)
                """
                # PMU
                ...
                # Avoiding CTIRQ1/2 assert
                self.write16(0x83070008, 0xABCD)
                self.write16(0x83070010, 0x0003)
    ...
    def brom_register_access(self, address, length, data=None, check_status=True, mode=-1):
        if mode == -1:
            if data is None:
                mode = 0
            else:
    ...
    def send_env_prepare(self, address, size, dadata):
        self.config.set_gui_status(self.config.tr("Sending ENV."))
        gen_chksum, data = self.prepare_data(dadata, b"", size)
        if not self.echo(self.Cmd.SEND_ENV_PREPARE.value):  # 0xD7
            self.error("Error on ENV cmd")
            self.config.set_gui_status(self.config.tr("Error on DA_Send cmd"))
            return False
        if not self.echo(address):
            self.error("Error on ENV address")
            self.config.set_gui_status(self.config.tr("Error on DA_Send address"))
            return False
        if not self.echo(len(data)):
            self.error("Error on ENV size")
            self.config.set_gui_status(self.config.tr("Error on DA_Send size"))
            return False
        if not self.echo(0x400A000):
            self.error("Error on ENV Addr1")
            self.config.set_gui_status(self.config.tr("Error on ENV Addr1"))
            return False
        if not self.echo(0x1000):
            self.error("Error on ENV Size1")
            self.config.set_gui_status(self.config.tr("Error on ENV Size1"))
            return False
        status = self.rword()
        if status != 0:
            return False
        if self.upload_data(data, gen_chksum):
            tmp = self.rdword()
            tmp2 = self.rword()
            self.usbwrite(b"\x5A")
            tmp = self.rword()
